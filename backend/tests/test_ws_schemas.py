...
def test_init_first_connect_keys_match():
    ...
    assert set(init.model_dump(exclude_none=True)) == {
        "type", "player_id", "depth", "grid", "width", "height", "traps", "items", "difficulty",
...
def test_init_floor_change_omits_player_id():
    ...
    assert set(dumped) == {
        "type", "depth", "grid", "width", "height", "traps", "items", "difficulty",
        "custom_tiles", "custom_walls", "torches",
    }
...
def test_state_update_keys_match():
    update = StateUpdateMessage(
        players=[], mobs=[], visible_tiles=[], events=[],
    )
    assert set(update.model_dump(exclude_none=True)) == {
        "type", "players", "mobs", "visible_tiles", "events",
    }
...
def test_state_update_passes_nested_payloads_through_untouched():
    ...
    update = StateUpdateMessage(
        players=[player], mobs=[], items=[item],
        visible_tiles=[[1, 2]], events=[event],
    )
