...
def _update_goo(game, goo: Goo, floor: FloorState, floor_id: int) -> bool:
    _goo_sync_enrage(game, goo, floor_id)
    _goo_water_heal(game, goo, floor, floor_id)

    target = game._find_nearest_player(goo.pos, floor_id)
    if target is None:
        _goo_cancel_charge(game, goo, floor_id)
        return False
    ...
    dist = game._get_distance(goo.pos, target.pos)
    charge_ok = dist <= 2 and game._is_in_los(goo.pos, target.pos, floor_id=floor_id)
    now = time.time()
    ...
    if goo.pumped_up >= 1:
        ...
        if now - goo.last_attack_time >= goo.attack_cooldown:
            ...
            goo.pumped_up += 1
            if goo.pumped_up >= 2:
                _goo_release_charge(game, goo, target, floor_id)
                goo.pumped_up = 0
        return True
    ...
    if charge_ok and now - goo.last_attack_time >= goo.attack_cooldown:
        ...
        if random.randint(0, odds - 1) == 0:
            goo.last_attack_time = now
            goo.pumped_up = 2 if "stronger_bosses" in game.challenges else 1
            _goo_begin_charge(game, goo, target, floor_id)
            return True
    return False


def _goo_sync_enrage(game, goo: Goo, floor_id: int):
    ...
    if enraged and not goo.enraged_announced:
        ...
        game.add_event("GOO_ENRAGE", {"mob": goo.id}, floor_id=floor_id)


def _goo_water_heal(game, goo: Goo, floor: FloorState, floor_id: int):
    ...
    removed = goo.heal_inc if "stronger_bosses" in game.challenges else goo.heal_inc * 1.5
    for player in game._players_on_floor(floor_id):
        if player.locked_floor_left is not None:
            player.locked_floor_left -= removed
    game.add_event("HEAL", {"target": goo.id, "amount": goo.heal_inc,
                            "x": goo.pos.x, "y": goo.pos.y}, floor_id=floor_id)
    if "stronger_bosses" in game.challenges:
        goo.heal_inc = min(3, goo.heal_inc + 1)


def _goo_seal_entrance(game, floor: FloorState, floor_id: int):
    ...
    floor.rebuild_flags()
    game.add_event("MAP_PATCH", {"tiles": [{"x": ex, "y": ey, "tile": TileType.FLOOR_WATER}]},
                   floor_id=floor_id)
    # SPD Buff.affect(hero, LockedFloor.class): 50 turns, 20 with stronger_bosses.
    left = 20.0 if "stronger_bosses" in game.challenges else 50.0
    for player in game._players_on_floor(floor_id):
        player.locked_floor_left = left


def _goo_unseal_entrance(game, floor: FloorState, floor_id: int):
    ...
    floor.rebuild_flags()
    game.add_event("MAP_PATCH", {"tiles": [{"x": ex, "y": ey, "tile": TileType.STAIRS_UP}]},
                   floor_id=floor_id)
    for player in game._players_on_floor(floor_id):
        player.locked_floor_left = None


def _goo_add_locked_floor_time(game, floor_id: int, player, dmg: int):
    ...
    if player is None or player.locked_floor_left is None or dmg <= 0:
        return
    added = dmg if "stronger_bosses" in game.challenges else dmg * 1.5
    player.locked_floor_left = min(50.0, player.locked_floor_left + added)


def _goo_threatened_tiles(game, goo: Goo, floor_id: int):
    tiles = []
    ...
    for dy in range(-radius, radius + 1):
        for dx in range(-radius, radius + 1):
            ...
            if game._is_in_los(goo.pos, Position(x=x, y=y), floor_id=floor_id):
                tiles.append([x, y])
    return tiles


def _goo_begin_charge(game, goo: Goo, target, floor_id: int):
    tiles = _goo_threatened_tiles(game, goo, floor_id)
    game.add_event("GOO_CHARGE", {"mob": goo.id, "tiles": tiles,
                                  "duration_ms": int(goo.attack_cooldown * 1000)},
                   floor_id=floor_id)
    game.add_event("PLAY_SOUND", {"sound": "CHARGEUP"}, floor_id=floor_id)


def _goo_cancel_charge(game, goo: Goo, floor_id: int):
    if goo.pumped_up > 0:
        goo.pumped_up = 0
        game.add_event("GOO_CHARGE", {"mob": goo.id, "tiles": []}, floor_id=floor_id)


def _goo_release_charge(game, goo: Goo, target, floor_id: int):
    game.add_event("GOO_CHARGE", {"mob": goo.id, "tiles": []}, floor_id=floor_id)
    ...
    game.add_event("SCREEN_SHAKE", {"intensity": 3, "duration_ms": 200}, floor_id=floor_id)
    ...
    if dmg > 0:
        ...
        game.add_event("PLAY_SOUND", {"sound": "HIT_BODY"}, floor_id=floor_id, source_player_id=target.id)
