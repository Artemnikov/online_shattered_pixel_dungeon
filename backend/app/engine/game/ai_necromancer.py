...
def _update_necromancer(game, necro: Necromancer, floor: FloorState, floor_id: int) -> bool:
    ...
    target = game._find_nearest_player(necro.pos, floor_id)

    if necro.summoning:
        _necro_summon_minion(game, necro, floor, floor_id)
        return True
    ...
    enemy_seen = game._is_in_los(necro.pos, target.pos, floor_id=floor_id,
                                   distance=game._view_distance(necro))

    if enemy_seen and game._get_distance(necro.pos, target.pos) <= 4 and skeleton is None:
        best_pos = None
        best_dist = float("inf")
        for dx, dy in _CIRCLE8_OFFSETS:
            ...
            if any(m.is_alive and m.pos.x == nx and m.pos.y == ny for m in floor.mobs.values()):
                continue
            if any(p.is_alive and p.pos.x == nx and p.pos.y == ny for p in game._players_on_floor(floor_id)):
                continue
            if not game._is_in_los(necro.pos, Position(x=nx, y=ny), floor_id=floor_id):
                continue
            d = game._get_distance(necro.pos, Position(x=nx, y=ny))
            if d < best_dist:
                best_dist = d
                best_pos = (nx, ny)
        if best_pos:
            ...
            game.add_event("ZAP_SUMMON", {"mob": necro.id, "x": best_pos[0], "y": best_pos[1]}, floor_id=floor_id)
        return True
    ...
    if enemy_seen and skeleton is not None:
        if skeleton.hp < skeleton.max_hp and necro.heal_cooldown == 0:
            ...
            game.add_event("HEAL", {"target": skeleton.id, "amount": heal,
                                      "x": skeleton.pos.x, "y": skeleton.pos.y}, floor_id=floor_id)
            game.add_event("ZAP_SUMMON", {"mob": necro.id, "x": skeleton.pos.x, "y": skeleton.pos.y}, floor_id=floor_id)
...
def _necro_summon_minion(game, necro: Necromancer, floor: FloorState, floor_id: int) -> None:
    ...
    if occupied_by is not None or impassable:
        ...
        if best:
            ...
            if occupied_by is not None and getattr(occupied_by, "faction", None) != necro.faction:
                ...
                if taken > 0:
                    game.add_event("DAMAGE", {"target": occupied_by.id, "amount": taken}, floor_id=floor_id)
            return
    ...
    if skeleton is None or not skeleton.is_alive:
        ...
        game.add_event("NECRO_SUMMON", {"necromancer": necro.id, "skeleton": new_skel.id, "x": x, "y": y}, floor_id=floor_id)
    else:
        skeleton.pos = Position(x=x, y=y)
        game.add_event("NECRO_SUMMON", {"necromancer": necro.id, "skeleton": skeleton.id, "x": x, "y": y}, floor_id=floor_id)
