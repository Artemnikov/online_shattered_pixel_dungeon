...
def _update_yog_dzewa(game, yog: YogDzewa, floor: FloorState, floor_id: int):
    if not yog.fight_started:
        ...
        if target is not None and game._get_distance(yog.pos, target.pos) <= 12:
    ...
    if yog.phase in (1, 2, 3) and yog.hp <= HP_FLOORS[yog.phase]:
        ...
        if yog.fist_order:
            ...
            new_fist = game._spawn_mob_at(fist_cls, yog.pos.x, yog.pos.y)
        ...
        yog.phase += 1
        floor.view_distance = _yog_phase_view_distance(yog.phase)
        game.add_event("YOG_PHASE_CHANGE", {"mob": yog.id, "phase": yog.phase},
                       floor_id=floor_id)
    ...
    if yog.ability_cooldown > 0:
        ...
        for _ in range(beams):
            ...
            if acu > df:
                ...
                game.add_event("ATTACK", {"source": yog.id, "target": target.id,
                                          "damage": taken, "surprise": False},
                               floor_id=floor_id)
        game.add_event("YOG_DEATH_RAY",
    ...
    if yog.summon_cooldown > 0:
        ...
        for sx, sy in neighbors:
            ...
            if not occupied:
                minion = game._spawn_mob_at(YogRipper, sx, sy)
