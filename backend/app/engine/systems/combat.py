...
BERSERK_POWER_DAMAGE_SCALE = 0.5
VULNERABLE_DAMAGE_MULTIPLIER = 1.33
DEATH_MARK_DAMAGE_MULTIPLIER = 1.25
DEGRADE_DAMAGE_MULTIPLIER = 0.85
GRIM_EXECUTE_FRACTION = 0.5                 # fraction of current HP dealt on a Grim proc
HEX_ACCURACY_MULTIPLIER = 0.75
DAZE_ACCURACY_MULTIPLIER = 0.5
HEX_DEFENSE_MULTIPLIER = 0.75
WEAKNESS_DAMAGE_MULTIPLIER = 0.67
BLESS_DAMAGE_MULTIPLIER = 1.2
EMPOWERED_STRIKE_SCALE_PER_LEVEL = 1.0 / 6.0
PROJECTILE_MOMENTUM_ACCURACY_SCALE = 0.5
PROJECTILE_MOMENTUM_DAMAGE_PER_LEVEL = 0.15
...
def _apply_talent_multipliers(effective: int, attacker: "Entity", defender: "Entity", result: dict, is_ranged: bool = False) -> int:
    ...
    if not is_ranged:
        ...
        if fs > 0 and getattr(attacker, "_last_action", None) == "ranged":
            effective = int(effective * (1.0 + fs * FOLLOWUP_STRIKE_BONUS_PER_LEVEL))

    # Point Blank (huntress T3): extra ranged damage at close range
    if is_ranged:
        pb = ti.level("point_blank")
        if pb > 0:
            effective = int(effective * (1.0 + pb * POINT_BLANK_BONUS_PER_LEVEL))
...
def _apply_post_dr_multipliers(raw_damage: int, attacker: "Entity", defender: "Entity", result: dict, is_ranged: bool = False) -> int:
    ...
    if attacker.has_fury:
        effective = int(effective * FURY_DAMAGE_MULTIPLIER)
    ...
    if berserk_active and berserk_power > 0:
        effective = int(effective * (1.0 + berserk_power * BERSERK_POWER_DAMAGE_SCALE))
...
def _check_grim(attacker: "Entity", defender: "Entity", result: dict):
    ...
    if random.random() < final_chance:
        extra_dmg = int(defender.hp * GRIM_EXECUTE_FRACTION)
        if extra_dmg > 0:
...
def resolve_melee_attack(
    ...
    if actual_damage > 0:
        ...
        if es_buff is not None:
            ...
            mult = 1.0 + es_level * EMPOWERED_STRIKE_SCALE_PER_LEVEL
...
def resolve_ranged_attack(
    ...
    if getattr(attacker, "invisible", 0) > 0 and _can_surprise_attack(attacker, weapon=item):
        ...
        if attacker.has_buff("hex"):
            attack_skill = int(attack_skill * HEX_ACCURACY_MULTIPLIER)
        ...
        if proj_momentum and getattr(attacker, "freerun_seconds", 0) > 0:
            attack_skill *= 1 + proj_momentum * PROJECTILE_MOMENTUM_ACCURACY_SCALE
        def_ev = defender.get_effective_defense_skill()
        if defender.has_buff("hex"):
            def_ev = int(def_ev * HEX_DEFENSE_MULTIPLIER)
    ...
    proj_momentum = getattr(attacker, "subclass_info", None) and attacker.subclass_info.talent_info.level("projectile_momentum")
    if proj_momentum and getattr(attacker, "freerun_seconds", 0) > 0:
        dmg_roll = round(dmg_roll * (1 + PROJECTILE_MOMENTUM_DAMAGE_PER_LEVEL * proj_momentum))
    if hasattr(attacker, "belongings"):
